	...
	<div id="wrap">
		...
		<div id="content" class="clearfix">
			<div id="profilecate_area">
				<div id="profile">
					<c:choose>
						<c:when test="${catepostMap.logoFile == null }">
							<!-- 기본이미지 -->
				...
				<div id="cate">
					...
					<c:forEach items="${catepostMap.cateList }" var="cateList">
						<ul id="cateList" class="text-left">
							<li><a href="${pageContext.request.contextPath}/${blogVo.id}?cateNo=${cateList.cateNo}">미분류</a></li>
						</ul>
					</c:forEach>
				</div>
			</div>
			...
			<div id="post_area">
				...
				<c:choose>
					<c:when test="${catepostMap.postVo != null }">
						<div id="postBox" class="clearfix">
							<div id="postTitle" class="text-left">
								<strong>${catepostMap.postVo.postTitle }</strong>
							</div>
							<div id="postDate" class="text-left">
								<strong>${catepostMap.postVo.regDate }</strong>
							</div>
							<div id="postNick">${blogVo.userName }(${blogVo.id })님</div>
						</div>
						<!-- //postBox -->

						<div id="post">${catepostMap.postVo.postContent }</div>
						<!-- //post -->
					</c:when>
					<c:otherwise>
						<!-- 글이 없는 경우 -->
				...
				<div id="list">
					<div id="listTitle" class="text-left">
						...
						<table>
							...
							<c:forEach items="${catepostMap.postList }" var="postList">
								<tr>
